...
pub enum RelaxError {
    ...
    UndefinedReference { span: codemap::Span, identifier: String },
    UndefinedDatatypePrefix { span: codemap::Span, prefix: String },
    NamespacePrefixAlreadyDefined { span: codemap::Span, prefix: String },
    ...
    StartRuleNotDefined { span: codemap::Span },
    /// the grammar has both 'foo |= ...' and 'foo &= ...' constructs
    DefineRulesMixesChoiceAndInterleave { this_span: codemap::Span, that_span: codemap::Span },
...
impl<'a> Context<'a> {
    ...
    fn new_include(&self, span: codemap::Span, file: Arc<codemap::File>) -> Result<Context, RelaxError> {
    ...
    fn file(&self) -> Arc<codemap::File> {
        match self {
            Context::Root { file, .. } | Context::Include { file, .. } => {
                file.clone()
            },
            ...
            Context::Grammar { parent, .. } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
                parent.file()
            }
        }
    }
    ...
    fn default_namespace_uri(&self) -> &str {
        match self {
            Context::Root { default_namespace, .. } | Context::Include { default_namespace, .. } => {
                &default_namespace[..]
            },
            Context::IncludeOverrides { parent, .. } | Context::Grammar { parent, .. } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
                parent.default_namespace_uri()
            },
        }
    }
    ...
    fn declare_namespace(&mut self, prefix: String, uri: String) -> Result<(), RelaxError> {
        match self {
            ...
            Context::IncludeOverrides { .. } | Context::Grammar { .. } | Context::Define { .. } | Context::Element { .. } | Context::Attribute { .. } => {
    ...
    fn namespace_uri_for_prefix_str(&self, prefix: &str) -> Option<&str> {
        match self {
            ...
            Context::IncludeOverrides { parent, .. } | Context::Grammar { parent, .. } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
                parent.namespace_uri_for_prefix_str(prefix)
            },
        }
    }
    ...
    fn try_datatype_uri_for_prefix(&self, prefix: &types::NcName) -> Result<&str, RelaxError> {
        self.datatype_uri_for_prefix(&prefix.1)
            .ok_or_else(|| RelaxError::UndefinedDatatypePrefix { span: self.convert_span(&prefix.0), prefix: prefix.1.clone() } )
    }
    ...
    fn datatype_uri_for_prefix(&self, prefix: &str) -> Option<&str> {
        match self {
            ...
            Context::Include { parent, datatypes, .. } => {
    ...
    fn define(&self, id: &str, rule: model::DefineRule) -> Result<(), RelaxError> {
        match self {
            ...
            Context::Include { parent, overrides, .. } => {
            ...
            Context::IncludeOverrides { parent } => {
                if let Context::Include { overrides, parent: parant_of_include, .. } = parent {
                    let mut overrides = overrides.borrow_mut();
                    if overrides.contains_key(id) {
    ...
    fn get_ref(&self, ref_id: &str) -> std::option::Option<std::rc::Rc<std::cell::RefCell<std::option::Option<model::DefineRule>>>> {
        match self {
            Context::Root { refs, .. } | Context::Grammar { refs, .. } => {
                refs.borrow().get(ref_id).map(Clone::clone)
            },
            Context::Include { parent, .. } | Context::IncludeOverrides { parent } | Context::Element { parent } | Context::Attribute { parent, .. } => parent.get_ref(ref_id),
            Context::Define { parent, id } => {
                if id.1 == ref_id {
                    panic!("unexpected ref lookup of {:?} within its own definition", id.1);
                }
    ...
    fn ref_iter(&self) -> impl Iterator<Item=(std::string::String, std::rc::Rc<std::cell::RefCell<std::option::Option<model::DefineRule>>>)> + '_ {
        match self {
            Context::Root { refs, .. } | Context::Grammar { refs, .. } => {
                refs.borrow().clone().into_iter().map(|(id, r)| (id.clone(), r.clone()) )
            },
    ...
    fn acquire_ref_impl(&self, ref_id: &types::Identifier) -> Result<model::PatRef, RelaxError> {
        match self {
            ...
            Context::Include { parent, .. } | Context::IncludeOverrides { parent } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
                parent.acquire_ref_impl(ref_id)
            }
        }
    }
    ...
    fn acquire_parent_ref(&self, id: &types::Identifier) -> Result<model::PatRef, RelaxError> {
        match self {
            ...
            Context::Include { parent, .. } | Context::IncludeOverrides { parent } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
                parent.acquire_parent_ref(id)
            }
            Context::Grammar { parent, .. } => {
                parent.acquire_ref(id)
            }
        }
    }
    ...
    fn parent_attribute(&self) -> Option<codemap::Span>{
        match self {
            Context::Root { .. } | Context::Grammar { .. } => {
                None
            },
            Context::Attribute { span, .. } => {
                Some(span.clone())
            }
            Context::Include { parent, .. } | Context::IncludeOverrides { parent } | Context::Define { parent, .. } | Context::Element { parent } => {
                parent.parent_attribute()
            }
        }
    }

    fn check_include(&self, span: codemap::Span, this_file: Arc<codemap::File>) -> Result<(), RelaxError> {
        match self {
            Context::Root { .. }  => {
                Ok(())
            },
            Context::Grammar { parent, .. } | Context::IncludeOverrides { parent } | Context::Define { parent, .. } | Context::Element { parent } | Context::Attribute { parent, .. } => {
...
impl<FS: Files> Compiler<FS> {
    ...
    pub fn compile(&mut self, name: &Path) -> Result<Rc<RefCell<Option<model::DefineRule>>>, RelaxError> {
        ...
        for (name, r) in ctx.ref_iter() {
            if r.borrow().is_none() {
                ...
                return Err(RelaxError::UndefinedReference { span: file.span.subspan(0, 0), identifier: name.clone() })
            } else {
        ...
        if let Some(start) = ctx.get_ref("start") {
            ...
            Err(RelaxError::StartRuleNotDefined { span: file.span.clone() })
        }
    }
    ...
    fn check(&self, seen: &mut HashSet<usize>, patt: &model::Pattern) -> Result<(), RelaxError> {
        match patt {
            Pattern::Choice(v) | Pattern::Interleave(v) | Pattern::Group(v) => {
                for p in v { self.check(seen, p)? }
            }
            Pattern::Mixed(p) | Pattern::Optional(p) | Pattern::ZeroOrMore(p) | Pattern::OneOrMore(p) | Pattern::Attribute(_, p) | Pattern::Element(_, p) | Pattern::List(p) => {
                self.check(seen, p)?
            },
            Pattern::Empty | Pattern::Text | Pattern::NotAllowed | Pattern::DatatypeValue { .. } => {
            },
            Pattern::Ref(span, name, def) => {
                ...
                if !seen.contains(&ptr) {
                    ...
                    if let Some(rule) = def.0.borrow().as_ref() {
                        ...
                        return Err(RelaxError::UndefinedReference { span: span.clone(), identifier: name.to_string() });
                    }
                }
            },
    ...
    fn diagnostic(&self, err: &RelaxError) -> codemap_diagnostic::Diagnostic {
        match err {
            ...
            RelaxError::DatatypeError(e) => match e {
                ...
                Errors::Xsd(e) => match e {
                    ...
                    XsdDatatypeError::UnsupportedDatatype { span, name } => {
                        ...
                        codemap_diagnostic::Diagnostic {
                            level: codemap_diagnostic::Level::Error,
                            message: format!("Unsupported datatype {:?} for XML Schema datatype library", name),
                            code: None,
                            spans: vec![label]
                        }
                    },
            ...
            RelaxError::RecursiveReference { ref_id, ref_span, def_id, def_span } => {
                let label = codemap_diagnostic::SpanLabel {
                    span: ref_span.clone(),
                    style: codemap_diagnostic::SpanStyle::Primary,
                    label: None,
            ...
            RelaxError::DuplicateDefinition { name, duplicate, original } => {
            ...
            RelaxError::DefineRulesMixesChoiceAndInterleave { this_span, that_span } => {
                let dup = codemap_diagnostic::SpanLabel {
                ...
                let orig = codemap_diagnostic::SpanLabel {
                    ...
                    style: codemap_diagnostic::SpanStyle::Secondary,
                    label: Some("Previously defined here with a different combine-rule".to_string()),
                };
                codemap_diagnostic::Diagnostic {
                    level: codemap_diagnostic::Level::Error,
                    message: "Definition mixes 'combine' and 'interleave' combine-rules".to_string(),
                    code: None,
                    spans: vec![dup, orig]
                }
            ...
            RelaxError::ElementAsChildOfAttribute { attribute_span, element_span } => {
            ...
            RelaxError::OverrideMissingFromInclude { override_span, include_span, name } => {
                ...
                let include_label = codemap_diagnostic::SpanLabel {
                    ...
                    style: codemap_diagnostic::SpanStyle::Secondary,
                    label: Some(format!("...or add a definition for {:?} to this grammar", name)),
                };
                codemap_diagnostic::Diagnostic {
                    level: codemap_diagnostic::Level::Error,
                    message: format!("Override {:?} has no corresponding definition in included grammar", name),
                    code: None,
    ...
    fn get_schema(&mut self, name: &Path) -> Result<(Arc<codemap::File>, Rc<Schema>), RelaxError> {
        ...
        let file = self.codemap.add_file(name.to_string_lossy().to_string(), data);
        let schema = self.syntax.parse(&file)?;
        let schema = Rc::new(schema);
        self.loaded.insert(name.to_path_buf(), (file.clone(), schema.clone()));
        Ok((file, schema))
    }
    ...
    fn compile_schema(&mut self, ctx: &mut Context, schema: Rc<Schema>) -> Result<(), RelaxError> {
        ...
        match schema.pattern_or_grammar {
            ...
            types::PatternOrGrammar::Grammar(types::GrammarPattern { span: _, ref content }) => {
    ...
    fn compile_define(&mut self, ctx: &mut Context, define: &types::Define) -> Result<(), RelaxError> {
        ...
        let new_rule = match define.2 {
            ...
            types::AssignMethod::Choice => model::DefineRule::CombineOnly(span, model::CombineRule::Choice, Pattern::Choice(vec![rule])),
            types::AssignMethod::Interleave => model::DefineRule::CombineOnly(span, model::CombineRule::Interleave, Pattern::Interleave(vec![rule])),
        };
        ctx.define(&id, new_rule)?;
        Ok(())
    }
    fn compile_grammar_div(&mut self, ctx: &mut Context, div: &[types::GrammarContent]) -> Result<(), RelaxError> {
        for item in div {
    ...
    fn compile_include(&mut self, ctx: &mut Context, inc: &types::Include) -> Result<(), RelaxError> {
        ...
        let span = ctx.file().span.subspan((inc.0).0.start as u64, (inc.0).0.end as u64);
        let (file, s) = self.get_schema(&path).map_err(|e| RelaxError::IncludeError(span, Box::new(e)))?;
        ...
        if let Some(ref inherit) = inc.1 {
            ...
            let namespace_uri = ctx
                ...
                .ok_or_else(|| RelaxError::UndefinedNamespacePrefix { span: ctx.convert_span(&inherit.0.span()), prefix: prefix.clone() })?
                .to_string();
            ...
            for o in overrides.iter() {
                match o {
                    types::IncludeContent::Define(d) => {
                        self.compile_define(&mut override_ctx, d).map_err(|e| RelaxError::IncludeError(span, Box::new(e)))?
                    },
                    types::IncludeContent::Div(d) => self.compile_include_div(&mut override_ctx, &d[..]).map_err(|e| RelaxError::IncludeError(span, Box::new(e)))?,
                    types::IncludeContent::Annotation(_) => (),
                }
            }
        }

        self.compile_schema(&mut inc_ctx, s).map_err(|e| RelaxError::IncludeError(span, Box::new(e)))?;

        if let Context::Include { overrides, .. } = inc_ctx {
            for (name, (override_span, defined)) in overrides.borrow().iter() {
                if !defined {
                    return Err(RelaxError::OverrideMissingFromInclude { name: name.clone(), override_span: override_span.clone(), include_span })
                }
            }
        } else {
            panic!()
    ...
    fn merge1(name: &str, a: model::DefineRule, b: model::DefineRule) -> Result<model::DefineRule, RelaxError> {
        ...
        match (a, b) {
            ...
            (DefineRule::CombineOnly(this, CombineRule::Choice, mut patt_a), DefineRule::CombineOnly(_, CombineRule::Choice, patt_b)) => {
                Self::append_choice(&mut patt_a, patt_b);
                Ok(DefineRule::CombineOnly(this, CombineRule::Choice, patt_a))
            },
            (DefineRule::AssignCombine(this, Some(CombineRule::Choice), mut patt_a), DefineRule::CombineOnly(_, CombineRule::Choice, patt_b)) |
            (DefineRule::CombineOnly(this, CombineRule::Choice, mut patt_a), DefineRule::AssignCombine(_, Some(CombineRule::Choice), patt_b)) |
            (DefineRule::AssignCombine(this, None, mut patt_a), DefineRule::CombineOnly(_, CombineRule::Choice, patt_b)) |
            (DefineRule::CombineOnly(this, CombineRule::Choice, mut patt_a), DefineRule::AssignCombine(_, None, patt_b)) => {
                ...
                Ok(DefineRule::AssignCombine(this, Some(CombineRule::Choice), result))
            }
            // interleave-related cases,
            (DefineRule::CombineOnly(this, CombineRule::Interleave, mut patt_a), DefineRule::CombineOnly(_, CombineRule::Interleave, patt_b)) => {
                Self::append_interleave(&mut patt_a, patt_b);
                Ok(DefineRule::CombineOnly(this, CombineRule::Interleave, patt_a))
            },
            (DefineRule::AssignCombine(this, Some(CombineRule::Interleave), mut patt_a), DefineRule::CombineOnly(_, CombineRule::Interleave, patt_b)) |
            (DefineRule::CombineOnly(this, CombineRule::Interleave, mut patt_a), DefineRule::AssignCombine(_, Some(CombineRule::Interleave), patt_b)) |
            (DefineRule::AssignCombine(this, None, mut patt_a), DefineRule::CombineOnly(_, CombineRule::Interleave, patt_b)) |
            (DefineRule::CombineOnly(this, CombineRule::Interleave, mut patt_a), DefineRule::AssignCombine(_, None, patt_b)) => {
                ...
                Ok(DefineRule::AssignCombine(this, Some(CombineRule::Interleave), result))
            }
            // invalid combinations,
            (DefineRule::CombineOnly(this_span, CombineRule::Choice, _), DefineRule::CombineOnly(that_span, CombineRule::Interleave, _)) |
            (DefineRule::CombineOnly(this_span, CombineRule::Interleave, _), DefineRule::CombineOnly(that_span, CombineRule::Choice, _)) |
            (DefineRule::AssignCombine(this_span, Some(CombineRule::Choice), _), DefineRule::CombineOnly(that_span, CombineRule::Interleave, _)) |
            (DefineRule::AssignCombine(this_span, Some(CombineRule::Interleave), _), DefineRule::CombineOnly(that_span, CombineRule::Choice, _)) |
            (DefineRule::CombineOnly(this_span, CombineRule::Choice, _), DefineRule::AssignCombine(that_span, Some(CombineRule::Interleave), _)) |
            (DefineRule::CombineOnly(this_span, CombineRule::Interleave, _), DefineRule::AssignCombine(that_span, Some(CombineRule::Choice), _)) => {
                return Err(RelaxError::DefineRulesMixesChoiceAndInterleave { this_span, that_span })
            }
        }
    }
    ...
    fn compile_ref(&mut self, ctx: &mut Context, reference: &types::Identifier) -> Result<model::Pattern, RelaxError> {
        let span = ctx.file().span.subspan((reference.0).start as u64, (reference.0).end as u64);
        Ok(Pattern::Ref(span, reference.1.clone(), ctx.acquire_ref(reference)?))
    }
    fn compile_parent(&mut self, ctx: &mut Context, reference: &types::Identifier) -> Result<model::Pattern, RelaxError> {
        let span = ctx.file().span.subspan((reference.0).start as u64, (reference.0).end as u64);
        Ok(Pattern::Ref(span, reference.1.clone(), ctx.acquire_parent_ref(reference)?))
    }
    fn compile_external(&mut self, ctx: &mut Context, external: &types::ExternalPattern) -> Result<model::Pattern, RelaxError> {
        ...
        match &s.pattern_or_grammar {
            types::PatternOrGrammar::Pattern(pat) => {
                self.compile_pattern(&mut inc_ctx, pat)
            ...
            types::PatternOrGrammar::Grammar(types::GrammarPattern { span: _, content }) => {
                ...
                for (name, r) in child_ctx.ref_iter() {
                    if r.borrow().is_none() {
                        ...
                        return Err(RelaxError::UndefinedReference { span: child_ctx.convert_span(&(0..0)), identifier: name.clone() })
                    }
                }
                if let Some(r) = child_ctx.get_ref("start") {
                    r.borrow_mut().take()
                        .map(|d| match d {
                            model::DefineRule::AssignCombine(_, _, p) | model::DefineRule::CombineOnly(_, _, p) => p,
                        })
                        .ok_or_else(|| RelaxError::StartRuleNotDefined { span: file_span.clone() })
                } else {
                    Err(RelaxError::StartRuleNotDefined { span: file_span.clone() })
                }
            }
        }
    }

    fn compile_grammar_pattern(&mut self, ctx: &mut Context, grammar: &types::GrammarPattern) -> Result<model::Pattern, RelaxError> {
        let mut child_ctx = ctx.new_grammar();
        ...
        if let Some(r) = child_ctx.get_ref("start") {
            r.borrow_mut().take()
                ...
                .ok_or_else(|| {
                    RelaxError::StartRuleNotDefined { span: ctx.convert_span(&grammar.span) }
                })
        } else {
            Err(RelaxError::StartRuleNotDefined { span: ctx.convert_span(&grammar.span) })
        }
    }

    fn compile_grammar_contents(&mut self, mut child_ctx: &mut Context, content: &[types::GrammarContent]) -> Result<(), RelaxError>  {
        for g in content {
            self.compile_grammar_content_item(&mut child_ctx, g)?;
        }
        Ok(())
    ...
    fn compile_grammar_content_item(&mut self, child_ctx: &mut Context, g: &types::GrammarContent) -> Result<(), RelaxError> {
        match g {
            types::GrammarContent::Define(ref d) => {
                self.compile_define(child_ctx, d)
            },
    ...
    fn compile_datatype_value_pattern(&mut self, ctx: &mut Context, datatype_value: &types::DatatypeValuePattern) -> Result<model::Pattern, RelaxError> {
        ...
        let name = datatype_value.0.as_ref().unwrap_or(&types::DatatypeName::Token);
        let name  = match name {
            DatatypeName::String => DatatypeName::String,
            DatatypeName::Token => DatatypeName::Token,
            DatatypeName::CName(QName(namespace, name)) => {
                DatatypeName::CName(QName(
                    NcName(namespace.0.clone(), ctx.try_datatype_uri_for_prefix(namespace)?.to_string()),
                    NcName(name.0.clone(), name.1.clone()),
                ))
            }
            DatatypeName::NamespacedName(NamespacedName { namespace_uri, localname }) => {
                DatatypeName::CName(QName(
    ...
    fn compile_datatype_name_pattern(&mut self, ctx: &mut Context, datatype_name: &types::DatatypeNamePattern) -> Result<model::Pattern, RelaxError> {
        ...
        let name  = match &datatype_name.0 {
            ...
            DatatypeName::CName(QName(namespace, name)) => {
                DatatypeName::CName(QName(
                    NcName(0..0, ctx.try_datatype_uri_for_prefix(namespace)?.to_string()),
                    NcName(name.0.clone(), name.1.clone()),
                ))
            }
            DatatypeName::NamespacedName(NamespacedName { namespace_uri, localname }) => {
    ...
    fn compile_declaration(&mut self, ctx: &mut Context, dec: &types::Decl) -> Result<(), RelaxError> {
        match dec {
            types::Decl::DefaultNamespace(types::DefaultNamespaceDeclaration { prefix, uri }) => {
                match uri {
                    NamespaceUriLiteral::Inherit => {
                        panic!("Can't inherit namespace {:?} at top level, I think?", prefix)
                    },
    ...
    fn compile_nameclass(&mut self, ctx: &mut Context, elem_attr: ElemAttr, name_class: &types::NameClass) -> Result<model::NameClass, RelaxError> {
        ...
        match name_class {
            types::NameClass::Name(name) => match name {
                ...
                Name::NamespacedName(NamespacedName { namespace_uri, localname }) => Ok(model::NameClass::named(
            ...
            types::NameClass::NsName(types::NsName { name, except } ) => {
                ...
                let uri = match name {
                    ...
                    types::NamespaceOrPrefix::Prefix(prefix) => ctx.namespace_uri_for_prefix(prefix)?.to_string(),
                };
        ...
        impl Files for FS {
            fn load(&self, name: &Path) -> Result<String, RelaxError> {
                let t = match name.to_str().unwrap() {
                    ...
                    _ => return Err(RelaxError::Io(name.to_path_buf(), io::Error::from(io::ErrorKind::NotFound))),
                };
                Ok(t.to_string())
            }
        }
